...
obs_est <- Est %>%
  filter(fecha > fin_ajuste_curva) %>%
  select(fecha, nuevos_obs_10, nuevos_obs_50, nuevos_obs_90) %>%
  mutate_at(.vars = c("nuevos_obs_10",
                      "nuevos_obs_50",
                      "nuevos_obs_90"),
            .funs = function(x) cumsum(x) + casos_acum_pre_ajuste) %>%
  rename(acum_obs_10 = nuevos_obs_10,
         acum_obs_50 = nuevos_obs_50,
         acum_obs_90 = nuevos_obs_90)
...
p1 <- bind_rows(Tab %>%
  ...
  mutate(modelo = factor(modelo, levels = c("real", "final_obs",
                                            "pre1_obs",
                                            "pre2_obs"))) %>%
  filter(!(q_90 > casos_acum_maximos) | is.na(q_90)) %>%
  # print(n = 1000)
